    ...
    class SimbadCli {
        ...
        private static void instanceFormatted(IEnumerable<DeepSpaceEntity> entities) {
            ...
            foreach (var entity in entities) {
                ...
                Console.WriteLine($"        raRate: new RateOfChange<RightAscension>(new RightAscension({entity.Coordinates.ProperMotion.RightAscensionRate.Amount.TotalHours()}), TimeSpan.FromSeconds(\"{entity.Coordinates.ProperMotion.RightAscensionRate.Duration.TotalSeconds()}\")),");
                Console.WriteLine($"        decRate: new RateOfChange<Declination>(new Declination({entity.Coordinates.ProperMotion.DeclinationRate.Amount.TotalDegrees()}), TimeSpan.FromSeconds(\"{entity.Coordinates.ProperMotion.DeclinationRate.Duration.TotalSeconds()}\")),");
                Console.WriteLine($"      }}");
                } else {
